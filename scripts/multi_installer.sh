...
# SpookyCoin Multi-installer
# a one line clone-and-compile for SpookyCoin:
#
#     ` $ curl -sL "https://raw.githubusercontent.com/SpookyCoin/SpookyCoin/master/scripts/multi_installer.sh" | bash
#
...
_set_wd() {
    if [ -d "$PWD/.git" ] && [ -f "$PWD/Makefile" ]; then
        ...
        if [ -d "$PWD"/SpookyCoin ]; then
            read -r -p "${1:-SpookyCoin directory already exists. Overwrite? [y/N]} " response
            case "$response" in
                [yY][eE][sS|[yY])
                    _colorize red "Overwriting old SpookyCoin directory" && echo
                    rm -rf "$PWD"/SpookyCoin
                    ;;
                *)
                    _fail "SpookyCoin directory already exists. Aborting..."
                    ;;
            esac
        fi
        mkdir SpookyCoin
        git clone -b master -q https://github.com/SpookyCoin/SpookyCoin SpookyCoin   >>build.log 2>&1 || _fail "Unable to clone git repository. Please see build.log for more information"
        cd SpookyCoin
    fi
}

_build_SpookyCoin() {
    _note "Building SpookyCoin from source (this might take a while)..."
    if [ -d build ]; then
...
_configure_linux() {
    if [ "$(awk -F= '/^NAME/{print $2}' /etc/os-release)" = "\"Ubuntu\"" ]; then
        ...
        _fail "Your OS version isn't supported by this installer. Please consider adding support for your OS to the project ('https://github.com/SpookyCoin')"
    fi
}
...
_configure_os() {
    ...
    case "${_unameOut}" in
        ...
        Darwin*)
            ...
            ;;
        *)
            _fail "This installer only runs on OSX 10.10+ and Ubuntu 16.04+. Please consider adding support for your OS to the project ('https://github.com/SpookyCoin')"
            ;;
    esac
    _note "Operating system configuration completed. You're halfway there!"
}

_note "SpookyCoin Multi_Installer v1.0 (pepperoni)"
...
_note "Look in 'SpookyCoin/build/src/' for the executible binaries. See 'https://github.com/SpookyCoin/SpookyCoin' for more project support. Cowabunga!"
